...
//Code Here
let evenNumbers = mixedNumbers.filter((num, index, arr) => {
  return num % 2 === 0;
});
...
//Code Here
let totalPopulation = populations.reduce((acc, el, index, arr) => {
  return (acc += el);
});
...
let bobsTotal = purchases.reduce((acc, el, index, arr) => {
  if (el.owner === "Bob") {
    return acc + el.price;
  } else {
    return acc;
